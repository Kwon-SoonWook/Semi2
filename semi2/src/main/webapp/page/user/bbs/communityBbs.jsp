...
ArrayList<BbsDTO> arr = bdao.userBbsList(select, input, 1);
int totalCnt = 0;
            ...
            <tfoot>
               <tr>
                  <td colspan='5' align='center'>
                     ...
                     if(arr==null || arr.size()==0){
                        %>&nbsp;&nbsp;<a
                        href="communityBbs.jsp?cp=1&input=<%=input%>&select=<%=select%>"
                        style="color: black; text-decoration: underline;">1</a>&nbsp;&nbsp;<%   
                     }else{
                        if (userGroup != 0) {
                           %> <a
                           href="communityBbs.jsp?cp=<%=(userGroup - 1) * pageSize + pageSize%>&input=<%=input%>&select=<%=select%>"
                        ...
                        for (int i = (userGroup * pageSize + 1); i <= (userGroup * pageSize + pageSize); i++) {
                           if (cp == i) {
                              %>&nbsp;&nbsp;<a
                              href="communityBbs.jsp?cp=<%=i%>&input=<%=input%>&select=<%=select%>"
                        ...
                        if (((totalPage / pageSize) - (totalPage % pageSize == 0 ? 1 : 0)) != userGroup) {
                           %> <a
                           href="communityBbs.jsp?cp=<%=(userGroup + 1) * pageSize + 1%>&input=<%=input%>&select=<%=select%>"
                           style="color: black; font-size: 10px; text-decoration: none">&gt;</a>
                           <%
                        }   
                     }

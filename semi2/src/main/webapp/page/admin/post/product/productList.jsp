...
//총 판매 게시글 수
ArrayList<ProductDTO> arr = pdao.productList(select, input);
int totalCnt;
...
<table>
	...
	<tbody>
		...
		%>
		<tr>
			<td style="text-align: center;"><%=arr.get(i).getProducts_id()%></td>			
			<td style="text-align: center;">판매</td>
			<td style="text-align: center;"><%=arr.get(i).getTitle()%></td>
			<td style="text-align: center;"><%=arr.get(i).getSeller_id()%></td>
			<td style="text-align: center;"><%=arr.get(i).getCreate_date()%></td>
			<td style="text-align: center;"><%=arr.get(i).getView_cnt()%></td>
			<td style="text-align: center;"><input type="checkbox" name='<%=arr.get(i).getProducts_id()%>'></td>
		<tr>
		<%
			if (i == totalCnt - 1)
				break;
	...
	<tfoot>
		<tr>
			<td colspan='7' align='center'>
				...
				if(arr==null || arr.size()==0){
					%>&nbsp;&nbsp;<a
					href="contentList.jsp?cp=1&category=판매 게시글&input=<%=input%>&select=<%=select_str%>"
					style="color: black; text-decoration: underline;">1</a>&nbsp;&nbsp;<%
				}
				else{
					if (userGroup != 0) {
					%> <a
					href="contentList.jsp?cp=<%=(userGroup - 1) * pageSize + pageSize%>&category=판매 게시글&input=<%=input%>&select=<%=select_str%>"
					...
					if (cp == i) {
					%>&nbsp;&nbsp;<a
					href="contentList.jsp?cp=<%=i%>&category=판매 게시글&input=<%=input%>&select=<%=select_str%>"
					...
					%> <a
					href="contentList.jsp?cp=<%=(userGroup + 1) * pageSize + 1%>&category=판매 게시글&input=<%=input%>&select=<%=select_str%>"
					style="color: black; font-size: 10px; text-decoration: none">&gt;</a>
					<%
					}
				}
